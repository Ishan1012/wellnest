...
export default function AppointmentsOverview() {
  ...
  const filteredAppointments = appointments.filter(appointment => {
    ...
    const matchesStatus = statusFilter === 'All' || (appointment as any).status === statusFilter;
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="w-full md:w-48">
          <select
            ...
            className="w-full bg-white border border-slate-200 rounded-xl py-3 px-4 text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:border-emerald-600 text-sm shadow-sm"
          >
                    ...
                    <React.Fragment key={app.id}>
                      <tr className="hover:bg-slate-50/30 border-b border-slate-100 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          ...
                          <div className="text-slate-400 text-xs mt-1 ml-5.5 ml-5">{app.time}</div>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wider ${getStatusBadgeClass((app as any).status || 'Scheduled')}`}>
                            {(app as any).status || 'Scheduled'}
                          </span>
                        </td>
                        ...
                        <tr className="bg-slate-50/40 border-b border-slate-100">
                          <td colSpan={6} className="px-8 py-5 text-slate-600">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              <div>
                                <h3 className="font-bold text-slate-850 text-slate-900 text-xs uppercase tracking-wider mb-2 border-b border-slate-150 pb-1">Patient Vitals & Contact</h3>
                              ...
                              <div>
                                <h3 className="font-bold text-slate-900 text-xs uppercase tracking-wider mb-2 border-b border-slate-150 pb-1">Consultation Intent</h3>
